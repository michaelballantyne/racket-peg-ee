  ...
  (define (dispatch-peg-compile e in)
    (syntax-parse e
      [_ #:when (peg-compile? e)
         (apply-as-transformer peg-compile 'expression e in)]))

  (define (dispatch-peg-expand e)
    (syntax-parse e
      [_ #:when (peg-expand? e)
         (apply-as-transformer peg-expand 'expression e)]
      [_ #:when (peg-macro? e)
         (dispatch-peg-expand
          (apply-as-transformer peg-macro 'expression e))]
      [nonterm:id (dispatch-peg-expand #'(#%peg-call nonterm))]
      [(head . rest) (raise-syntax-error #f "not a peg expression" e)]
      [d (with-syntax ([#%peg-datum (datum->syntax #'d '#%peg-datum)])
           (dispatch-peg-expand #'(#%peg-datum d)))])))
